...
exports.updateDescriptor = updateDescriptor;
exports.deleteMap = deleteMap;
exports.reset = reset;

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var ParseDescriptor = require('../parse_descriptor.js');
var model = ParseDescriptor.model;
var Validator = require('../validator.js');

function listDescriptor(req, res) {
...
function updateDescriptor(req, res) {
    if (!req.body) {
        res.sendStatus(401);
        return;
    }
    ...
    try {
        ...
        var count;
        model.findOne({ CAN_Id: Number(req.params.descriptor) }).exec(function (err, document) {
            if (document) {
                count = document.map.length;
                if (document.map.length > request.map.length) throw new Error("too few fields");
                for (var i = 0; i < count; i++) {
                    (0, _keys2.default)(request.map[i]).forEach(function (key) {
...
function deleteMap(req, res) {
    if (req.query.offset && req.query.length && req.query.description && req.query.dataType) {
        ...
        console.log(element);
        model.find({ CAN_Id: Number(req.params.descriptor), map: { $elemMatch: element } }, { map: 1, _id: 0 }).exec(function (err, doc) {
...
function reset(req, res) {
    ParseDescriptor.reset(function (err) {
        if (err) {
            console.error(err);
            res.sendStatus(401);
            return;
